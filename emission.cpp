...
bool Emission:: ajouter()
{
    ...
    QSqlQuery query;
          ...
          query.prepare("INSERT INTO EMISSIONS(id,nomem,typeem,dateem,duree) "
...
QSqlQueryModel *Emission::rechercher(QString rech)
{
    QSqlQueryModel * model= new QSqlQueryModel();

        model->setQuery("select * from Emissions where ID LIKE '"+rech+"%'");
    return model;
}
...
QSqlQueryModel *Emission::tri(){
   ...
   QSqlQueryModel *model = new QSqlQueryModel();
   q->prepare("SELECT * FROM Emissions ORDER BY DUREE ");
   q->exec();
   model->setQuery(*q);
...
QSqlQueryModel *Emission::trid(){

   QSqlQuery *q = new QSqlQuery();
   QSqlQueryModel *model = new QSqlQueryModel();
   q->prepare("SELECT * FROM Emissions ORDER BY DUREE DESC");
   q->exec();
   model->setQuery(*q);
