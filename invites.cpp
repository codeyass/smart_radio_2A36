...
bool INVITES::modifier()
{
        ...
        query.bindValue(":CIN_I",CIN_I);
        query.bindValue(":DATE_I",DATE_I);
        query.bindValue(":DUREE_I",DUREE_I);
        query.bindValue(":MAIL_I",MAIL_I);
        query.bindValue(":NUM_TEL_I",NUM_TEL_I);
       return query.exec();
...
QSqlQuery INVITES::request(QString code)
{
    QSqlQuery query;
    query.prepare("select * from CARTERFID where NUMCARTE= '"+code+"'");
