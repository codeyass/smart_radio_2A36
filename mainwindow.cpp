...
void MainWindow::on_ajouter_2_clicked()
{
    int id=ui->le_id_e->text().toInt();
            ...
            ui->tab_equipement->setModel(eq.afficher());
                ...
                msgBox.setText("AJOUT AVEC SUCCEES");
            ui->tab_equipement->setModel(eq.afficher());
...
void MainWindow::on_pushButton_supp_7_clicked()
{
    QSqlQueryModel * model= new QSqlQueryModel();
                       model->setQuery("select * from EMPLOYES where AGE <20 ");
                       float dispo1=model->rowCount();
                       ...
                       float total=dispo1+dispo;
                           QString a=QString("age<20  " +QString::number((dispo1*100)/total,'f',2)+"%" );
                           QString b=QString("age>20  " +QString::number((dispo*100)/total,'f',2)+"%" );
                           QPieSeries *series = new QPieSeries();
                           series->append(a,dispo1);
                           series->append(b,dispo);
                       if (dispo1!=0)
